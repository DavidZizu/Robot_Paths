...
void Square::odomCallback(const nav_msgs::OdometryConstPtr &odom) {
	...
	if (!rotation) {
		...
		if (angle < 0 ? (odom->pose.pose.orientation.w < cos((angle + M_PI / 2) / 2) && odom->pose.pose.orientation.w < 0.99999) : (odom->pose.pose.orientation.w > cos((angle + M_PI / 2) / 2) && odom->pose.pose.orientation.w > 0.005)) { 
			pub_vel.publish(vel_to_publish);
